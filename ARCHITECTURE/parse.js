...
const overflowed = quote => quote["error"] !== undefined;
if (overflowed(due)) 
{
	// console.error(`./parse.js: ${due["error"]}`)
	process.exit(1);
}
...
let routes = due.priceRoute.bestRoute.map(route => route.swaps.map(swaps => swaps.swapExchanges.map(swap => {
	// console.log("SWAP.DATA: ", swap.data)
	return {
		//destAmount: swap.destAmount,
		//blockPercent: route.percent,
		//swapPercent: swap.percent,
		pools: swap.poolAddresses,
		totalPercent: (route.percent * swap.percent), //200 represents 2%
		data: swap.data
	};
	//return swap.data
})))
...
cMap(5, "address[]", "pools"); //identity crisis | should be renamed to pools
reroute(5, 2, "pools", "pools");
...
let encode = () => {
	...
	let data = [...routes].map(route => {
		...
		let percent = route.totalPercent //Edit percents here
