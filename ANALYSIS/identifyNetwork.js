...
if (network === "undefined") {
  ...
  process.exit(1);
} else console.log("Analysing: ", network);
const sourceFiles = {
...
const getChain = (net) => {
  ...
  const chain = chains.filter(
    (chain) =>
      ...
      match(chain?.name?.toLowerCase()) ||
      match(chain?.chain?.toLowerCase()) ||
      match(chain?.chainSlug?.toLowerCase()) ||
      match(chain?.nativeCurrency.name?.toLowerCase()) ||
      match(chain?.nativeCurrency?.symbol?.toLowerCase())
  )[0];
  return chain;
};
...
const printData = (output, data) =>
  fs.writeFile(output, data, (err) =>
    console.log(`${output} update ${err ? "failed" : "succeeded"}`)
  );
...
const printReport = (file, data) =>
  fs.writeFile(file, data, (err) =>
    console.log(`${file} update ${err ? "failed" : "succeeded"}`)
  );
